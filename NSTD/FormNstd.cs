...
namespace Nstd
{
    ...
    {
        private const string Cs = @"Data Source=LAPTOP-KOTI7GRO\SQLEXPRESS;Initial Catalog=World;Integrated Security=True";
            ...
            using (var cn = new SqlConnection(Cs))
                {
                    ...
                    {
                        Connection = cn,
                        CommandText = @"SELECT field_name, table_name, field_type, transl_fn, category_name FROM fields ORDER BY category_name"
                    };
        ...
        private void cbNameField_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (i >= 0)
            {
                ...
                using (var sConn = new SqlConnection(Cs))
                {
                    sConn.Open();
                    var sCommand = new SqlCommand
                    {
                ...
                if (fieltype == "text" || fieltype == "varchar" || fieltype == "character varying" || fieltype == "char" || fieltype == "character")
                {
                    cbviroj.Visible = true;
                    dtdat.Visible = false;
        ...
        void Addvir(string tablename1, string tablename2, List<string> rela)
        {
            using (var sConn = new SqlConnection(Cs))
            {
                ...
                {
                    Connection = sConn,
                    CommandText = @"SELECT relations FROM rel_table
        ...
        void find(string tablename1, string tablename2, List<string> tranzitnamestables, List<string> rel)
        {
            ...
            using (var sConn = new SqlConnection(Cs))
            {
                ...
                {
                    Connection = sConn,
                    CommandText = @"SELECT table1, table2, relations, via FROM rel_table
                             WHERE table1 = @name_table1 AND table2 = @name_table2
                                OR table1 = @name_table2 AND table2 = @name_table1;"
                };
                sCommand.Parameters.AddWithValue("@name_table1", tablename1);
